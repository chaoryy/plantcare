...
router.post("/analyze", auth, upload.single("image"), async (req, res) => {
  ...
  try {
    const { base64, mimeType } = await compressImage(req.file.buffer);
    const result = await ai.identify(base64, mimeType);

    const virtualFilename = `${Date.now()}-${Math.round(Math.random() * 1e9)}.jpg`;

    await pool.query(
      "INSERT INTO analyses (user_id, type, image_url, result) VALUES ($1, $2, $3, $4)",
      [req.user.id, "identify", virtualFilename, result],
    );
    res.json(result);
  } catch (err) {
...
router.post("/diagnose", auth, upload.single("image"), async (req, res) => {
  ...
  try {
    ...
    else if (result.severity === "средняя" || result.severity === "высокая")
      status = "sick";
    ...
    await pool.query(
      "INSERT INTO analyses (user_id, type, image_url, result) VALUES ($1, $2, $3, $4)",
      [req.user.id, "diagnose", virtualFilename, result],
    );
...
router.post("/recommend", auth, async (req, res) => {
  ...
  if (!conditions)
    return res.status(400).json({ error: "Условия обязательны" });
  try {
...
router.post("/collection", auth, async (req, res) => {
  ...
  if (!plant_name)
    return res.status(400).json({ error: "Название растения обязательно" });
  try {
